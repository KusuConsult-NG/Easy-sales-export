const admin = require("firebase-admin");
admin.initializeApp({
  credential: admin.credential.cert({
    projectId: "easy-sales-hub",
    clientEmail: "firebase-adminsdk-fbsvc@easy-sales-hub.iam.gserviceaccount.com",
    privateKey: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
  })
});

async function run() {
  const db = admin.firestore();
  
  const landListings = await db.collection("land_listings").get();
  console.log("LAND_LISTINGS count:", landListings.size);
  if (landListings.size > 0) {
      console.log("Sample LAND_LISTINGS doc:", landListings.docs[0].id, Object.keys(landListings.docs[0].data()));
  }

  const farmNationProperties = await db.collection("farm_nation_properties").get();
  console.log("FARM_NATION_PROPERTIES count:", farmNationProperties.size);
  if (farmNationProperties.size > 0) {
      console.log("Sample FARM_NATION_PROPERTIES doc:", farmNationProperties.docs[0].id, Object.keys(farmNationProperties.docs[0].data()));
  }
}
run().catch(console.error);
