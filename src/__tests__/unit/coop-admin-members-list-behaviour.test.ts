...
describe('requestCooperativeRevisionAction', () => {
    ...
    it('refuses an admin without cooperatives:approve_members', async () => {
        ...
        // admitted by it. That was narrowed; then #955 removed the retry entirely,
        // because asking the record only when the TOKEN said no left the
        // newly-demoted admin admitted on a claim the record had already withdrawn.
        actAs('content-admin', ['academy_admin']);
        store.seed(COLLECTIONS.USERS, 'content-admin', { roles: ['academy_admin'] });

        //   The refusal is unchanged; the sentence is. requireAdmin names the
        //   permission it checked, which is the more actionable of the two — and
        //   asserting the permission means this cannot pass on a generic refusal
        //   from somewhere else in the action.
        const refused = await request('m1', 'Please redo');
        expect(refused).toMatchObject({ success: false });
        expect(String((refused as { error?: string }).error))
