...
describe('#328 — eslint sees scripts/', () => {
    ...
    it('POSITIVE CONTROL: it still ignores what it is supposed to', () => {
        ...
        /**
         * next-env.d.ts is the ignored file to test against — but it is
         * GENERATED by `next dev`/`next build` and gitignored, so on a checkout
         * that has never built, it does not exist and eslint answers "No files
         * matching the pattern" rather than "File ignored". The control then
         * fails for the absence of a file rather than for a broken ignore list,
         * which is a positive control that cannot tell the two apart.
         *
         * Written on disk for the duration of the check, so the control tests
         * the ignore list on any checkout, built or not.
         */
        const probe = join(process.cwd(), 'next-env.d.ts');
        const preexisting = existsSync(probe);
        if (!preexisting) writeFileSync(probe, '/// <reference types="next" />\n');
        try {
            const { output } = lint(['next-env.d.ts']);
            expect(output).toMatch(/File ignored/);
        } finally {
            if (!preexisting) unlinkSync(probe);
        }
    }, 180_000);
});
