...
describe('#328 — eslint sees scripts/', () => {
    ...
    it('POSITIVE CONTROL: it still ignores what it is supposed to', () => {
        /*
         * Otherwise "nothing is ignored" could mean the parse found no messages
         * at all and the assertion above is vacuous.
         *
         *   #701 THIS CONTROL POINTED AT A FILE THAT IS NOT IN THE REPOSITORY,
         *   AND IT BLOCKED A PRODUCTION DEPLOY.
         *
         *   It linted `next-env.d.ts` under the comment "next-env.d.ts exists on
         *   disk". It does not: the file is GENERATED by Next and is gitignored
         *   (.gitignore:41). It exists on a machine that has run a build, and on
         *   no other.
         *
         *   So the result depended on whether a build had happened first, and
         *   the two workflows differ in exactly that: ci.yml runs `npm run
         *   build` before its tests, and deploy-production.yml's Final
         *   Validation goes npm ci → tsc → lint → test with no build at all. The
         *   test therefore passed in CI, always, and failed in the release gate,
         *   always — a green CI that cannot see the failure that stops releases.
         *
         *   Its failure was also unreadable, which is how it survived: eslint
         *   answers a missing path with "Oops! Something went wrong! :( No files
         *   matching the pattern" rather than anything about ignoring.
         *
         *   EVERY ENTRY in globalIgnores is a generated path — .next, out, build,
         *   coverage, next-env.d.ts — so there is no committed file to point at
         *   instead. The control makes its own, inside an ignored directory, and
         *   removes it. That is deterministic on any machine, built or not.
         */
        const probeDir = join(ROOT, 'coverage');
        const probe = join(probeDir, '__eslint-ignore-probe.js');
        const dirExisted = existsSync(probeDir);

        try {
            if (!dirExisted) mkdirSync(probeDir, { recursive: true });
            //   Deliberately contains a lint error, so "ignored" is the only
            //   reason eslint could stay quiet about it.
            writeFileSync(probe, 'var unused = (\n');

            const { output } = lint(['coverage/__eslint-ignore-probe.js']);
            expect(output).toMatch(/File ignored/);
        } finally {
            //   The probe goes; a real coverage report does not.
            if (existsSync(probe)) rmSync(probe);
            if (!dirExisted && existsSync(probeDir)) rmSync(probeDir, { recursive: true });
        }
    }, 180_000);
});
