...
export default function CourseCatalogClient({ initial = null }: {
     ...
     *
     *   Raw, because the effect turns the enrolment rows into a Set of course
     *   ids — derivation that stays here, in one place.
     */
    initial?: {
    ...
    async function loadData() {
        ...
        try {
            /**
             *   #547 TWO INDEPENDENT READS THAT RAN ONE AFTER THE OTHER.
             *
             *   The catalogue and the member's enrolments do not depend on
             *   each other, and they were awaited in series — so a member
             *   waited the SUM of two round trips to see one screen.
             *
             *   Parallel now, and seeded from the server when it supplied
             *   them, in which case there is no round trip at all.
             */
            const seed = takeSeed();
            const [coursesRes, enrollRes] = seed
                ? [seed.coursesRes, seed.enrollRes]
                : await Promise.all([
                    getCoursesAction(48),
