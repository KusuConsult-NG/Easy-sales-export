...
export default function MyCoursesClient({ initial = null }: {
    /**  #546 Fetched by the server — see page.tsx. Seeded into state, so the
     *   server-rendered HTML already lists the courses. */
    initial?: EnrolledCourseWithDetails[] | null;
}) {
    const [courses, setCourses] = useState<EnrolledCourseWithDetails[]>(initial ?? []);
    const [loading, setLoading] = useState(initial === null);

    useEffect(() => {
        //   #546 Already supplied by the server; nothing to ask for.
