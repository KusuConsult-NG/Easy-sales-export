...
export default function ProgressClient({ initial = null }: {
    /**
     *   #550 The two RAW results the server already fetched.
     *
     *   Raw, because the effect flattens the aggregate into nine fields with a
     *   default for each — a mapping that stays here in one place.
     */
    initial?: { aggResult: any; streakResult: any } | null;
}) {
    ...
    useEffect(() => {
        async function loadProgress() {
            ...
            try {
                const seed = takeSeed();
                const [aggResult, streakResult] = seed
                    ? [seed.aggResult, seed.streakResult]
                    : await Promise.all([
