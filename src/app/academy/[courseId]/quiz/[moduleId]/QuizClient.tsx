...
export default function QuizClient(
    { courseId, moduleId, initial = null }: {
        courseId: string;
        moduleId: string;
        /**
         *   #562 The course RESULT, not the quiz. loadQuiz picks the module out
         *   of the course and the quiz out of the module, and #492 records that
         *   a refused read must not read as "quiz not found" — on a timed
         *   assessment, of all screens. Both stay on this side.
         */
        initial?: Awaited<ReturnType<typeof getCourseByIdAction>> | null;
    },
) {
    ...
    //   Taken once: the effect re-runs on a session change and on a route
    //   change to another module, and those runs must read the real course.
    ...
    async function loadQuiz() {
        ...
        setLoadError(null);
        try {
            const courseReq = takeSeed() ?? await getCourseByIdAction(courseId);
