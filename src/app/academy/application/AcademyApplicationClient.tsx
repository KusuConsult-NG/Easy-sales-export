 ...
 *
 *   #562 ONLY the two reads EVERY branch makes. The application itself is not
 *   seeded, and that is deliberate: which branches read it depends on
 *   `?edit=true`, on the payment answer, and on whether a submitted application
 *   exists — three conditions this screen resolves in a careful order that #316
 *   is written all over. Seeding it would mean restating that order on the
 *   server, which is the defect class this audit keeps finding. Two of up to
 *   three round trips go; the third stays where the rule lives.
 */
export type AcademyApplicationSeed = {
    status: Awaited<ReturnType<typeof checkAcademyStatusAction>>;
    payment: Awaited<ReturnType<typeof checkAcademyPaymentStatusAction>>;
};

export default function AcademyApplicationClient(
    { initial = null }: { initial?: AcademyApplicationSeed | null },
) {
    ...
    //   #562 — the two reads every branch of the chain below makes. Taken once
    //   each, so a session transition re-runs the effect against the real
    //   answers rather than against what was true when the page was rendered.
    const takeStatus = useServerSeed(initial?.status ?? null);
    const takePayment = useServerSeed(initial?.payment ?? null);
    ...
    useEffect(() => {
        ...
        const readPaymentStatus = async (): Promise<"paid" | "unpaid" | null> => {
            const res = takePayment() ?? await checkAcademyPaymentStatusAction();
        ...
        const checkStatus = async () => {
            try {
                const status = takeStatus() ?? await checkAcademyStatusAction();
