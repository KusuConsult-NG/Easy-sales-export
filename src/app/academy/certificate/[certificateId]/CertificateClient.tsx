...
export default function CertificateClient({ initial = null }: {
    /**
     *   #549 The course and the member's progress, both already fetched.
     *
     *   Seeded into state: the effect only assigns them, so the certificate
     *   arrives rendered rather than after a round trip.
     */
    initial?: { course: Course | null; progress: UserProgress | null } | null;
}) {
    ...
    const [progress, setProgress] = useState<UserProgress | null>(initial?.progress ?? null);
    ...
    useEffect(() => {
        ...
        //   #549 Already supplied by the server. The sign-in redirect below
        //   stays reachable: an unauthenticated visitor gets a null seed.
