...
export default function AcademyLiveClient({ initial = null }: {
    /**
     *   #548 The RAW session list the server already fetched.
     *
     *   Raw, because the effect splits it into "live" and "ended with a
     *   recording" — a rule that stays in one place.
     */
    initial?: Awaited<ReturnType<typeof getLiveSessionsAction>> | null;
}) {
    ...
    useEffect(() => {
        ...
        async function loadLiveSessions() {
            try {
                const liveRes = takeSeed() ?? await getLiveSessionsAction();
