...
export default function AcademyLiveClassClient(props: {
    /**
     *   #549 The id as a plain string. The SERVER page resolves the route
     *   params, so this component no longer unwraps a promise just to learn
     *   what to fetch — and does not have to be handed one back.
     */
    courseId: string;
    /**
     *   #549 The two RAW action results the server already fetched.
     *
     *   Raw, because the effect picks the ACTIVE session out of the list and
     *   falls back to a synthesised course title when the lookup misses — two
     *   rules that stay here, in one place.
     */
    initial?: { courseReq: any; liveRes: any } | null;
}) {
    ...
    useEffect(() => {
        ...
        async function loadCourse() {
            try {
                /**
                 *   #549 TWO INDEPENDENT READS THAT RAN IN SERIES.
                 *
                 *   The course and its live sessions do not depend on each
                 *   other — both are keyed on courseId — and they were awaited
                 *   one after the other, so the member waited the sum of two
                 *   round trips. Parallel now, and skipped entirely when the
                 *   server supplied them.
                 */
                const seed = takeSeed();
                const [courseReq, liveRes] = seed
                    ? [seed.courseReq, seed.liveRes]
                    : await Promise.all([
                        getCourseByIdAction(courseId),
                        getLiveSessionsAction(courseId),
                    ]);
        ...
        loadCourse();
    }, [status, courseId, router, takeSeed]);
