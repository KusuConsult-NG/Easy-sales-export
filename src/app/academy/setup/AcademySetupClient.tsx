 ...
 *
 * `payment` is null unless the application status is one that asks for it —
 * the same condition the client uses — so a learner who is not in that state
 * costs the same two reads they always did.
 */
export type AcademySetupSeed = {
    profile: Awaited<ReturnType<typeof getUserProfileAction>>;
    status: Awaited<ReturnType<typeof checkAcademyStatusAction>>;
    payment: Awaited<ReturnType<typeof checkAcademyPaymentStatusAction>> | null;
};
...
export default function AcademySetupClient({ initial = null }: { initial?: AcademySetupSeed | null }) {
    ...
    //   #558 — the server walked this chain already. Taken once, so a session
    //   transition re-runs the real reads.
    ...
    useEffect(() => {
        ...
        const checkStatus = async () => {
            ...
            try {
                /**
                 *   THE PROFILE AND THE APPLICATION STATUS HAVE NOTHING TO DO
                 *   WITH EACH OTHER, AND WERE AWAITED ONE AFTER THE OTHER.
                 *
                 *   The status does not depend on the profile and the profile
                 *   does not depend on the status, so the learner waited the SUM
                 *   of two round trips rather than the longer of them. Only the
                 *   THIRD read — the payment check — is genuinely conditional,
                 *   and it stays conditional. Same fix as #543 gave the WAVE and
                 *   Academy dashboards.
                 */
                // 1. Load structured name from Firestore root (NOT from JWT split)
                const [profileResult, status] = seed
                    ? [seed.profile, seed.status]
                    : await Promise.all([
                ...
                // 2. Act on the application status
                ...
                if (status.data === "approved" || status.data === "active") {
                    ...
                    const payStatus = seed?.payment ?? await checkAcademyPaymentStatusAction();
