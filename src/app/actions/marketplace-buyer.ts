...
export interface ProductFilters { category?: string;
    minPrice?: number;
    maxPrice?: number;
    state?: string;
    lga?: string;
...
async function _getProductsAction(filters?: ProductFilters) { let sessionResult;
    try {
        ...
        if (filters?.state) { query = query.where("location.state", "==", filters.state);
        }

        if (filters?.bulkAvailable) { query = query.where("bulkAvailable", "==", true);
        }

        if (filters?.exportReady) { query = query.where("exportReady", "==", true);
        }
        ...
        if (filters?.minPrice !== undefined || filters?.maxPrice !== undefined) { products = products.filter(product => {
        ...
        if (filters?.searchTerm) { const term = filters.searchTerm.toLowerCase();
        ...
        if (filters?.lga) { products = products.filter(product => product.location.lga === filters.lga);
        }
 ...
 */
async function _getProductByIdAction(productId: string) { let sessionResult;
    try {
        sessionResult = await requireSession().catch(() => ({ session: null }));
        ...
        if (!productDoc.exists) { return { success: false as const, error: "Product not found", data: null };
        }

        return { error: null, success: true as const, data: null };
    } catch (error) { logger.error("Get product error:", {
            userId: sessionResult?.session?.user?.id,
            productId,
            error: error instanceof Error ? error.message : String(error)
        });
 ...
 */
async function _getFeaturedProductsAction() { let sessionResult;
    try {
        sessionResult = await requireSession().catch(() => ({ session: null }));
        ...
        return { error: null, success: true as const, data: null };
    } catch (error) { logger.error("Get featured products error:", {
            userId: sessionResult?.session?.user?.id,
            error: error instanceof Error ? error.message : String(error)
        });
 ...
 */
async function _getProductsByCategoryAction(category: string) { let sessionResult;
    try {
        sessionResult = await requireSession().catch(() => ({ session: null }));
    ...
    } catch (error) { logger.error("Get products by category error:", {
            userId: sessionResult?.session?.user?.id,
            category,
            error: error instanceof Error ? error.message : String(error)
        });
...
async function _getBuyerOrdersAction() { let sessionResult;
    try {
...
async function _confirmOrderReceiptAction(orderId: string) { let sessionResult;
    try {
        sessionResult = await requireSession();
        if (!sessionResult.session) return { success: false as const, error: sessionResult.error.error, data: null };
        const { session } = sessionResult;
        ...
        // 2. Verify Buyer
        if (orderData?.buyerId !== userId) { return { success: false as const, error: "Unauthorized", data: null };
        }

        if (orderData?.status !== "in_transit" && orderData?.status !== "processing" && orderData?.status !== "shipped") { return { success: false as const, error: "Order is not yet in transit or processing", data: null };
        }

        await db.runTransaction(async (transaction) => { // 3. Update Order Status
            ...
            escrowQuery.docs.forEach(doc => { transaction.update(doc.ref, {
                    status: "released",
    ...
    } catch (error) { logger.error("Confirm receipt error:", {
            userId: sessionResult?.session?.user?.id,
            orderId,
            error: error instanceof Error ? error.message : String(error)
        });
        return { success: false as const, error: error instanceof Error ? error.message : "Failed to confirm receipt", data: null };
    }
}
