...
export async function setupTestCooperativeAction() { try {
        ...
        if (!cooperativeDoc.exists) { // Create the cooperative
        ...
        if (memberDoc.exists) { return { error: "You are already a member of this cooperative", success: false as const, data: null };
        }
        ...
        await cooperativeRef.update({ memberCount: FieldValue.increment(1),
        ...
        if (userDoc.exists) { await userRef.update({
                cooperativeId,
                updatedAt: FieldValue.serverTimestamp() });
        }

        return { error: null, success: true as const, message: "Successfully set up test cooperative! Refresh the page to see your membership." , data: null };
    } catch (error: any) {
