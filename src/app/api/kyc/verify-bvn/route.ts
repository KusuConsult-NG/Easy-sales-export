...
async function verifyBVNHandler(req: NextRequest) {
    try {
         ...
         *   say — "the member supplied a WELL-FORMED BVN and the platform has
         *   recorded it, unchecked". Nothing here checked well-formedness. The
         *   only test was `!bvn`, so "1" reached the success branch and came
         ...
         */
        const idCheck = nationalIdField('BVN').safeParse(String(bvn).trim());
        if (!idCheck.success) {
            return NextResponse.json(
                { error: idCheck.error.issues[0]?.message ?? 'Invalid BVN' },
                { status: 400 }
            );
        }
