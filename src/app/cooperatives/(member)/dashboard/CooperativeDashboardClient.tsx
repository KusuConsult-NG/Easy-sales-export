...
export default function CooperativeDashboardClient({ initial = null }: {
    /**  #543 Fetched by the server, seeded into state — see page.tsx. */
    initial?: { membership: CooperativeMembership | null; transactions: CooperativeTransaction[] } | null;
}) {
    ...
    const [membership, setMembership] = useState<CooperativeMembership | null>(initial?.membership ?? null);
    const [transactions, setTransactions] = useState<CooperativeTransaction[]>(initial?.transactions ?? []);
    ...
    useEffect(() => {
        ...
        //   #543 Already supplied by the server; nothing to ask for.
