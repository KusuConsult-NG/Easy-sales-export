...
export default function FixedSavingsClient({ initial = null }: { initial?: FixedSavingsSeed | null }) {
    ...
    const [plans, setPlans] = useState<FixedSavingsPlan[]>((initial?.plans ?? []) as FixedSavingsPlan[]);
     ...
     *
     *   checkMembership's catch set membershipStatus to "not_member", and the
     *   render below turns that into an onboarding panel headed "To access this
     *   feature, you must first become an approved cooperative member", with
     *   the member's own savings plans hidden behind it.
     *
     *   So one failed request — a blip, an expired session, a 500 — told a
     *   paid-up cooperative member they were not one and offered to sell them a
     *   membership they already had. On a money screen. The #323 shape again,
     *   and the third page in this codebase to have it.
     *
    ...
    useEffect(() => {
        //   #564 — the server made both reads before the page was sent, through
        //   the same functions the two routes use. A null seed means one of
        //   them failed, and then this asks over HTTP exactly as it always did.
        if (initial) return;
        checkMembership();
        fetchPlans();
    ...
    //   #565 A failed check is shown as a failure. Before this it fell into
                ...
                <div className="max-w-3xl mx-auto px-4">
                    ...
                    <div className="p-5 bg-amber-50 border border-amber-200 rounded-2xl text-amber-900">
                        ...
                        <p className="text-sm mt-1">
                            This is a problem on our side, not with your account. Your savings
                            plans are safe. Please reload the page in a moment.
                        </p>
                    </div>
                </div>
            </div>
