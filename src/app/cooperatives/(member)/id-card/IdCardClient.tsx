...
type IdCardResult = Awaited<ReturnType<typeof getCooperativeMemberIdCardAction>>;

/**
 * Everything the screen derives from the read, in ONE place.
 *
 *   #559 The seed and the refetch both go through this. Written out twice —
 *   once to initialise state from the server's answer and once inside the
 *   loader — it would be two copies of one contract, which is the defect class
 *   this audit keeps finding.
 *
 *   The editor opens by itself when the card is missing a field it needs, and
 *   that rule lives here rather than being restated at the seed.
 */
function derive(res: IdCardResult | null) {
    const card = res?.success ? res.data : null;
    return {
        gender: card?.gender || "",
        stateOfOrigin: card?.stateOfOrigin || "",
        editing: !!card && (!card.gender || !card.stateOfOrigin),
    };
}

export default function IdCardClient({ initial = null }: { initial?: IdCardResult | null }) {
    const router = useRouter();
    const cardRef = useRef<HTMLDivElement>(null);
    const { showToast } = useToast();
    //   #559 — the server already read the card, so the first paint carries it.
    const seed = derive(initial);

    const [loading, setLoading] = useState(initial === null);
    const [downloading, setDownloading] = useState(false);
    const [result, setResult] = useState<IdCardResult | null>(initial);

    const [isEditing, setIsEditing] = useState(seed.editing);
    const [editGender, setEditGender] = useState(seed.gender);
    const [editState, setEditState] = useState(seed.stateOfOrigin);
    const [saving, setSaving] = useState(false);

    //   Taken once: saving gender and state calls fetchData again, and that run
    //   must read the row it just wrote.
    const takeSeed = useServerSeed(initial);

    //   useCallback, not a plain arrow: these close over the take-once seed
    //   now, so the effect below genuinely depends on them and the dependency
    //   array should say so rather than be silenced.
    const apply = useCallback((res: IdCardResult) => {
        const next = derive(res);
        setResult(res);
        if (res.success && res.data) {
            setEditGender(next.gender);
            setEditState(next.stateOfOrigin);
            if (next.editing) setIsEditing(true);
        }
        setLoading(false);
    }, []);
    ...
    const fetchData = useCallback(() => {
        if (takeSeed()) return;
        setLoading(true);
        getCooperativeMemberIdCardAction().then(apply);
    }, [takeSeed, apply]);
