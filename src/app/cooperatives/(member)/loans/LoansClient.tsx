...
export type LoansSeed = {
    membership: { isMember: boolean; status: string };
    products: Record<string, unknown>[];
    applications: Record<string, any>[];
};

export default function LoansClient({ initial = null }: { initial?: LoansSeed | null }) {
    const router = useRouter();

    //   #570 — the server made all three reads through the same functions the
    //   three routes use.
    const seededMembership = initial ? membershipAnswerFrom({ success: true, ...initial.membership }) : null;

    const [products, setProducts] = useState<LoanProduct[]>((initial?.products ?? []) as LoanProduct[]);
    const [applications, setApplications] = useState<LoanApplication[]>((initial?.applications ?? []) as LoanApplication[]);
    ...
    const [membershipStatus, setMembershipStatus] = useState<"approved" | "pending" | "not_member" | null>(
        seededMembership?.known ? seededMembership.status : null,
    );
    /**
     *   #570 "COULD NOT TELL" IS NOT "NOT A MEMBER" — the second screen.
     *
     *   This carried a byte-for-byte copy of the code #565 fixed on
     *   /cooperatives/fixed-savings: a catch that set membershipStatus to
     *   "not_member", which renders a panel headed "you must first become an
     *   approved cooperative member" — here, over the member's own loan
     *   applications and repayment figures.
     *
     *   THE FIX REACHED ONE OF TWO DOORS. It reached one because each screen
     *   carried its own copy of the reading. Both now call
     *   membershipAnswerFrom, so there is no second copy to fix next time.
     */
    const [membershipCheckFailed, setMembershipCheckFailed] = useState(false);
    const [selectedProduct, setSelectedProduct] = useState<LoanProduct | null>(null);

    useEffect(() => {
        //   A null seed means one of the three server reads failed; the client
        //   then asks over HTTP exactly as it always did.
        if (initial) return;
        checkMembership();
        fetchProducts();
        fetchApplications();
    ...
    const checkMembership = async () => {
        try {
            ...
            const answer = membershipAnswerFrom(await response.json());
    ...
    //   #570 A failed check is shown as a failure. Before this it fell into
    //   the onboarding panel below and read as "you are not a member".
            ...
            <div className="min-h-screen bg-slate-50 py-8">
                <div className="max-w-3xl mx-auto px-4">
                    ...
                    <div className="p-5 bg-amber-50 border border-amber-200 rounded-2xl text-amber-900">
                        <p className="font-semibold">We could not check your cooperative membership</p>
                        <p className="text-sm mt-1">
                            This is a problem on our side, not with your account. Any loan you
                            already hold is unaffected. Please reload the page in a moment.
                        </p>
                    </div>
                </div>
            </div>
