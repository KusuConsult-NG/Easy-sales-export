...
export default function MySavingsClient({ initial = null }: { initial?: MySavingsSeed | null }) {
    //   #564 — the server made both reads. Handed INTO the effect rather than
    //   into useState because this screen DERIVES from the plans: it reshapes
    //   each one into the card the list renders, and the projected profit it
    //   shows is the SERVER's stored figure, not one worked out here. That
    ...
    const loadSavings = useCallback(async () => {
        ...
        try {
            ...
            const [membershipResult, savingsData] = seeded
                ? [seeded.membership, { success: true, plans: seeded.plans }]
                : await Promise.all([
                    getMembershipAction(),
                    fetch("/api/cooperative/fixed-savings").then(r => r.ok ? r.json() : { success: false }),
                ]);
            ...
            //   The `savingsResponse.ok` branch that used to wrap this is gone:
            //   the fetch now resolves to { success: false } on a bad status, so
            //   there is one failure path instead of two nested ones.
            {
