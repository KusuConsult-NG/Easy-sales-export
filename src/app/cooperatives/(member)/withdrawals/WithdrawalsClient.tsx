...
export default function WithdrawalsClient({ initial = null }: { initial?: Withdrawal[] | null }) {
    //   #564 — the server already read these. Taken once, so any later run
    //   reads the real list.
    const takeSeed = useServerSeed(initial);

    const [loading, setLoading] = useState(initial === null);
    const [withdrawals, setWithdrawals] = useState<Withdrawal[]>(initial ?? []);
    const [timeFilter, setTimeFilter] = useState("all");
    const [statusFilter, setStatusFilter] = useState("all");

    //   useCallback because it closes over the take-once seed now.
    const loadWithdrawals = useCallback(async () => {
        const seeded = takeSeed();
        if (seeded) {
            setWithdrawals(seeded);
            setLoading(false);
            return;
        }
        ...
        try {
            /**
             *   #566 A ROUND TRIP TO ASK A QUESTION THE NEXT CALL ANSWERS
             *        ANYWAY.
             *
             *   This fetched /api/auth/session first, purely to check that a
             *   user existed, and returned early if not. getMyWithdrawals
             *   resolves the session ITSELF and returns [] without one — its
             *   own first two lines — so the check decided nothing that the
             *   call below did not decide again a moment later.
             *
             *   What it cost was a whole HTTP round trip from the browser back
             *   to this application, in front of every load of this screen, and
             *   an early `return` that left the spinner running forever when
             *   the session endpoint was the thing that failed.
             *
