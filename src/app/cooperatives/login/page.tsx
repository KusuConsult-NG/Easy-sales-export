...
import { Mail, Lock, AlertCircle, Eye, EyeOff, Building2, CheckCircle, ArrowRight, Loader2 } from "lucide-react";
...
function CooperativeLoginContent() {
    const { showToast } = useToast();
    const searchParams = useSearchParams();
    const callbackUrl = searchParams.get("callbackUrl") || "/cooperatives/dashboard";
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex items-center justify-center p-4 relative overflow-hidden">
            {/* Background Pattern - Matching Registration */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <div className="absolute -top-32 -left-32 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl opacity-50" />
                <div className="absolute bottom-0 -right-32 w-96 h-96 bg-indigo-500/10 rounded-full blur-3xl opacity-50" />
            ...
            <div className="relative w-full max-w-md">
                ...
                <div className="text-center mb-8 relative z-10">
                    <Link href="/" className="inline-flex items-center justify-center mb-6 hover:opacity-90 transition-opacity">
                        <div className="w-16 h-16 bg-linear-to-br from-purple-800 to-indigo-900 rounded-2xl flex items-center justify-center shadow-xl shadow-purple-500/20 text-white transform rotate-3 hover:rotate-6 transition-transform">
                            <Building2 className="w-8 h-8" />
                        </div>
                    </Link>
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-2">Welcome Back</h1>
                    <p className="text-slate-500 dark:text-slate-400">Sign in to your Cooperative account</p>
                </div>
                    ...
                    <form action={formAction} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-purple-600 transition-colors" />
                                <input
                                    id="login-email"
                                    type="email"
                                    name="email"
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative group">
                                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-purple-600 transition-colors" />
                                <input
                                    id="login-password"
                            ...
                            <label className="flex items-center gap-3 cursor-pointer group">
                                <div className="relative flex items-center">
                                    <input
                                        ...
                                        className="peer w-5 h-5 rounded-md border-2 border-slate-300 dark:border-slate-600 text-purple-600 focus:ring-purple-500/50 transition-all active:scale-95"
                                        disabled={isPending}
                                    />
                        ...
                        <LoadingButton
                            ...
                            className="w-full py-2.5 bg-blue-900 hover:bg-blue-950 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all border-0 flex items-center justify-center gap-2"
                        >
                            Sign In
