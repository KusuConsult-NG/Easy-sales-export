...
export default function CooperativesOnboardingError({
    error,
    reset,
}: {
    ...
    useEffect(() => {
        if (isStaleDeploymentError(error)) {
            console.warn("[CooperativesOnboardingError] Stale deployment detected — auto-reloading.", error.name, error.message);
            window.location.reload();
            return;
        }
        console.error("[Cooperatives Onboarding Error]", error);
    }, [error]);
        ...
        <div className="min-h-[400px] flex items-center justify-center p-6 bg-slate-50">
            <div className="max-w-md w-full bg-white border border-red-100 rounded-2xl p-8 shadow-sm text-center">
                ...
                <p className="text-slate-500 mb-8 text-sm leading-relaxed">
                    We encountered an unexpected error while loading Cooperatives onboarding. Our technical team has been notified.
                </p>
                <div className="flex flex-col gap-3 justify-center items-center">
                    <div className="flex flex-col sm:flex-row gap-3 w-full">
                        ...
                        <button
                            onClick={() => window.location.href = "/cooperatives"}
