...
type MembershipSeed = Awaited<ReturnType<typeof getMembershipAction>>;

export default function CooperativePaymentClient(
    { initial = null }: { initial?: MembershipSeed | null },
) {
    ...
    //   #560 — the server already read the membership this gate turns on.
    //   Taken once: a session transition re-runs the effect and must read the
    //   real row, because paying elsewhere changes the answer.
    ...
    useEffect(() => {
        ...
        const checkExisting = async () => {
            try {
                const response = takeSeed() ?? await getMembershipAction();
        ...
        checkExisting();
    }, [sessionStatus, router, showToast, prefix, takeSeed]);
