...
export type CertificatesSeed = {
    uploaded: Record<string, any>[];
    academy: Record<string, any>[];
};

export default function CertificatesClient({ initial = null }: { initial?: CertificatesSeed | null }) {
    ...
    async function fetchAll() {
        //   #562 — the server read both lists before the page was sent.
        const seeded = takeSeed();
        if (seeded) {
            setCertificates(seeded.uploaded as Certificate[]);
            setAcademyCerts(seeded.academy as AcademyCertificate[]);
            setIsLoading(false);
            return;
        }
        ...
        try {
             ...
             *        SCREEN OPENS ON.
             *
             *   The two endpoints do not answer in the same shape.
             *   /api/certificates replies { success, certificates } — flat.
             *   /api/academy/certificates replies { success, data: {
             *   certificates }, meta } — enveloped. This file read
             *   `acData.certificates` for BOTH, so the academy half was
             *   `undefined || []` on every load, forever.
             *
             *   Not a rare path: `activeTab` starts at "academy", so this is
             *   what a learner sees the moment they open the page. And the
             *   route above it carries a comment — #425 — recording that the
             *   academy half of every learner's certificate list used to be
             *   empty for a different reason, which was found and fixed. The
             *   list was still empty. THE FIX REACHED ONE OF TWO DOORS.
             *
             *   Read from where the route actually puts them. The seeded path
             *   above cannot make this mistake at all, because the shared
