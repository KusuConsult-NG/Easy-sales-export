...
export default function DisputesClient({ initial = null }: {
    /**
     *   #548 The disputes the server already fetched, seeded into state.
     *
     *   This screen also POLLS (#545 put it on the visibility-aware primitive),
     *   so the seed must not be treated as "done" — the poll keeps running and
     *   keeps the list current. What it must do is skip the FIRST fetch, which
     *   the server has already made.
     */
    initial?: Dispute[] | null;
}) {
    ...
    useEffect(() => {
        ...
        //   #548 `immediate: false` when the server already supplied the list:
        //   the poll is a refresh from here, not the first read.
        const stopPolling = startVisibilityAwareInterval(
            fetchDisputes, 10000, { immediate: initial === null },
        );
