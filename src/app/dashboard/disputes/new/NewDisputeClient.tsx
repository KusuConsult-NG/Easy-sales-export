...
function NewDisputePageContent({ initial }: { initial: Order | null }) {
    ...
    useEffect(() => {
        //   #550 Already supplied by the server.
...
export default function NewDisputeClient({ initial = null }: {
    /**
     *   #550 The order the server already fetched.
     *
     *   `loadOrder` stays reachable: it is what redirects to the orders list
     *   when the order is missing or not the caller's, and a null seed — which
     *   is what a refusal produces — still runs it.
     */
    initial?: Order | null;
}) {
    return (
        <Suspense fallback={
            ...
            <NewDisputePageContent initial={initial} />
        </Suspense>
    );
}
