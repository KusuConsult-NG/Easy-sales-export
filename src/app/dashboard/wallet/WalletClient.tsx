 ...
 *
 * Three separate results rather than one flattened object: each loader below
 * does its own derivation from its own result — the withdrawal bank from the
 * wallet, the paging cursor from the last transaction — and that derivation
 * stays exactly where it was.
 */
export type WalletSeed = {
    toggles: Awaited<ReturnType<typeof getFeatureTogglesAction>> | null;
    wallet: Awaited<ReturnType<typeof getWalletAction>> | null;
    transactions: Awaited<ReturnType<typeof getWalletTransactionsAction>> | null;
};
...
export default function WalletClient({ initial = null }: { initial?: WalletSeed | null }) {
    ...
    //   #558 — the server already made all three of these reads. One take-once
    //   seed each, because each loader is called again on its own schedule: the
    //   wallet after funding or withdrawing, the transactions when paging.
    const takeToggles = useServerSeed(initial?.toggles ?? null);
    const takeWallet = useServerSeed(initial?.wallet ?? null);
    const takeTransactions = useServerSeed(initial?.transactions ?? null);

    const loadToggles = useCallback(async () => {
        const res = takeToggles() ?? await getFeatureTogglesAction();
        if (res.success && res.data) setToggles(res.data);
    }, [takeToggles]);

    const loadWallet = useCallback(async () => {
        const seeded = takeWallet();
        //   Only show the spinner when there is actually something to wait for.
        if (!seeded) setLoading(true);
        const res = seeded ?? await getWalletAction();
    ...
    const loadTransactions = useCallback(async (reset = false) => {
        //   The seed is the FIRST page, so it is only good for a reset. Paging
        //   asks for the next one exactly as before.
