...
export default function EscrowDetailClient({ escrowId, initial = null }: {
    /**
     *   #547 The id comes as a plain prop now: the SERVER page resolves the
     *   route params and does the read, so this component no longer unwraps a
     *   promise just to find out what to fetch.
     */
    escrowId: string;
    initial?: EscrowDetail | null;
}) {
    const [escrow, setEscrow] = useState<EscrowDetail | null>(initial);
    ...
    useEffect(() => {
        //   #547 Already supplied by the server. `load` stays exported to the
        //   rest of the component, which calls it to refresh after an action.
