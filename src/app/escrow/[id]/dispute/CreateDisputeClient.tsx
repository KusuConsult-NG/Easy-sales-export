...
export default function CreateDisputeClient(props: {
    /**  #553 The id as a plain string — the server page resolved the params. */
    escrowId: string;
    /**
     *   The escrow the server already fetched. `loadEscrow` stays: it is what
     *   shows "Escrow transaction not found" and redirects, and a refusal
     *   passes null so that path still runs.
     */
    initial?: EscrowTransaction | null;
}) {
    ...
    const [escrowData, setEscrowData] = useState<any>(props.initial ?? null);
    const [loadingEscrow, setLoadingEscrow] = useState(props.initial == null);
    const escrowId = props.escrowId;
