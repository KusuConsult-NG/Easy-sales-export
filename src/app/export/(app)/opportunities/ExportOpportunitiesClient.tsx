...
export default function ExportOpportunitiesClient({ initial = null }: {
    /**
     *   #546 The RAW action result. The effect below turns date strings back
     *   into Date objects, which is derivation the server must not duplicate —
     *   and could not hand over anyway, since a Date does not survive the
     *   server-to-client boundary as a Date.
     */
    initial?: Awaited<ReturnType<typeof getActiveExportWindowsAction>> | null;
}) {
    ...
    async function loadWindows() {
        ...
        setError(null);
        try {
            const result = takeSeed() ?? await getActiveExportWindowsAction();
            if (result.success) {
