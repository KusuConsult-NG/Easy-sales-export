...
export default function ExportTransactionsClient({ initial = null }: {
    /**
     *   #546 The RAW action result the server already fetched.
     *
     *   Raw, not derived: the effect below turns each investment into two rows
     *   and sorts them, and running that on the server too would be two copies
     *   of one contract. So the seed replaces the FETCH and the derivation
     *   stays here, exactly once — which means the first paint still shows this
     *   screen's loading state and the rows appear at hydration rather than a
     *   round trip later.
     */
    initial?: Awaited<ReturnType<typeof getMyExportInvestmentsAction>> | null;
}) {
    ...
    useEffect(() => {
        const seed = takeSeed();
        (seed ? Promise.resolve(seed) : getMyExportInvestmentsAction()).then((result) => {
            if (result.success ) {
