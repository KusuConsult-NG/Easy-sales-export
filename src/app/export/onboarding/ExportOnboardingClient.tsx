 ...
 *
 * `application` and `hasAccess` follow the SAME branch the client takes, so
 * each is null unless the status the server saw actually calls for it. Nothing
 * is read speculatively.
 */
export type ExportOnboardingSeed = {
    status: string | null;
    application: Awaited<ReturnType<typeof getExportApplicationAction>> | null;
...
export default function ExportOnboardingClient(
    { initial = null }: { initial?: ExportOnboardingSeed | null },
) {
    ...
    //   #558 — the server walked this chain already, following the same
    //   branches. Taken once, so the session transition this effect exists to
    //   wait for still does the real reads if it comes round again.
    ...
    useEffect(() => {
        ...
        const checkStatus = async () => {
            ...
            try {
                ...
                const status = seed ? seed.status : await checkExportStatusAction();
