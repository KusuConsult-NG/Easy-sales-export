...
export default function ExportWindowDetailClient({ initial = null }: {
    /**
     *   #548 The window the server already fetched.
     *
     *   `loadWindow` stays: this screen reloads itself after an investment, so
     *   the client keeps its own read for the refresh even though the FIRST one
     *   now arrives with the HTML.
     */
    initial?: ExportOpportunity | null;
}) {
    ...
    const [windowData, setWindowData] = useState<ExportOpportunity | null>(initial);
    const [loading, setLoading] = useState(initial === null);
    const [investing, setInvesting] = useState(false);
    /**
     *   #548 SEEDED TOO, AND THIS ONE NEARLY GOT MISSED.
     *
     *   loadWindow sets BOTH windowData and this field — to the window's
     *   minInvestment. Seeding only the window and skipping the load would have
     *   left the invest box defaulted to 0 instead of the minimum: the screen
     *   would render correctly and the form would be wrong, which is precisely
     *   the failure mode #545 warns a conversion can introduce without turning
     *   a single test red.
     */
    const [investmentAmount, setInvestmentAmount] = useState<number>(initial?.minInvestment ?? 0);
    ...
    useEffect(() => {
        //   #548 Already supplied by the server.
        if (initial !== null) return;
        if (windowId) {
            loadWindow();
        }
    }, [windowId, loadWindow, initial]);
