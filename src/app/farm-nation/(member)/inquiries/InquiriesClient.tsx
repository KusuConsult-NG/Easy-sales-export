...
export default function InquiriesClient({ initial = null }: {
    /**  #549 The inquiries the server already fetched, seeded into state. */
    initial?: any[] | null;
}) {
    const router = useRouter();
    const { data: session, status } = useSession();
    const [inquiries, setInquiries] = useState<any[]>(initial ?? []);
    ...
    useEffect(() => {
        ...
        //   #549 Already supplied by the server. A refusal passes null and
        //   this still runs, so the error message keeps saying itself.
