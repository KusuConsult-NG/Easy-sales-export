...
export default function FarmNationLandingClient({ initial = null }: {
    /**
     *   #553 The RAW listing search the server already ran.
     *
     *   Raw, because the effect takes the first three as "featured", counts the
     *   rest by category and derives a total — three derivations that stay here
     *   rather than being restated on the server.
     */
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                const result = takeSeed() ?? await searchLandListingsAction({ limit: 50 });
