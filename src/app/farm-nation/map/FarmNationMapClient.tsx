...
/**
 *   #562 Exported so the server half can seed this component without inventing
 *   a second name for the same shape. There are two other `LandListing` types
 *   in the tree — one in types/strict and one in packages/farm-nation — and
 *   this screen deliberately uses neither; picking the wrong one at the seeding
 *   boundary is how a server page and its client come to disagree.
 */
...
export default function FarmNationMapClient(
    { initial = null }: { initial?: LandListing[] | null },
) {
    ...
    //   #562 — the server read the listings through the same shared reader the
    //   HTTP route uses. Handed INTO the effect rather than into useState,
    //   because this screen DERIVES from them — it keeps only the parcels that
    //   are purchasable and have GPS coordinates, a rule that was wrong twice
    //   before and must not be restated on the server.
    ...
    async function fetchListings() {
        ...
        try {
            const data = seeded
                ? { success: true, data: { listings: seeded } }
                : await (await fetch("/api/farm-nation/listings")).json();
