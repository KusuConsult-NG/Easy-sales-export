...
export default function LandMapClient({ initial = null }: { initial?: LandListing[] | null }) {
    //   #562 — the server read these before the page was sent, so the map
    //   arrives with its pins on it. `initial` is null only when that read
    //   failed, and then this falls back to the HTTP route exactly as before.
    const [listings, setListings] = useState<LandListing[]>(initial ?? []);
