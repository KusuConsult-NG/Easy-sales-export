...
export default function BuyerDashboardClient({ initial = null }: {
    /**
     *   #543 The three action RESULTS the server already fetched.
     *
     *   The raw results, not derived state. This effect sorts the orders and
     *   reshapes the recommendations, and doing that on the server as well
     *   would be two copies of one contract — the defect class this audit keeps
     *   finding. So the seed replaces the FETCH and the derivation stays here,
     *   exactly once.
     *
     *   The cost of that choice, stated plainly: the server-rendered HTML still
     *   shows this screen's loading state. What goes away is the network round
     *   trip, so the numbers appear as soon as the bundle hydrates instead of a
     *   further round trip later.
     */
    initial?: { statsResult: any; ordersResult: any; recommendedResult: any } | null;
}) {
    ...
    useEffect(() => {
        async function loadDashboard() {
            try {
                const seed = takeSeed();
                const [statsResult, ordersResult, recommendedResult] = seed
                    ? [seed.statsResult, seed.ordersResult, seed.recommendedResult]
                    : await Promise.all([
                        getBuyerStatsAction(),
