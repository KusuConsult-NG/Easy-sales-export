...
export default function BuyerOrdersClient({ initial = null }: {
    /**
     *   #554 The FIRST, unfiltered page. Take-once and guarded: this screen
     *   refetches on the status filter and pages with a cursor.
     */
    initial?: Awaited<ReturnType<typeof getBuyerOrdersAction>> | null;
}) {
    ...
    const fetchOrders = useCallback(async (isReset: boolean = false) => {
        try {
            ...
            const seeded = (isReset && !filterStatus) ? takeSeed() : null;
            const result = seeded ?? await getBuyerOrdersAction({
                limit: 20,
