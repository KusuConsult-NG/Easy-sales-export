...
export default function BuyerOrderDetailClient({ initial = null }: {
     ...
     *
     *   Only the ORDER. The tracking effect below keys off
     *   `order.trackingNumber`, which is not known until the order is in hand,
     *   so that read genuinely depends on this one and stays in the client.
     */
    initial?: Order | null;
}) {
    ...
    useEffect(() => {
        //   #554 Already supplied by the server. A refusal passes null and this
        //   runs, so "Order not found" is still the client's own message.
