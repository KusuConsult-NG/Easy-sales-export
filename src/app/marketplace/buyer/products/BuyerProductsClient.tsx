...
export default function BuyerProductsClient({ initial = null }: {
    /**
     *   #554 The two RAW results, already fetched in parallel.
     *
     *   Raw and take-once: the effect maps flash-sale rows into a Product-like
     *   shape — a substantial mapping that stays in one place — and it re-runs
     *   on the state filter, the price range and a debounced search, so the
     *   seed answers only the initial unfiltered load.
     */
    initial?: { productsRes: any; flashRes: any } | null;
}) {
    ...
    const fetchProducts = useCallback(async () => {
        ...
        try {
            ...
            const unfiltered = selectedState === "all" && !searchQuery && !priceRange;
            const seed = unfiltered ? takeSeed() : null;
            const [productsRes, flashRes] = seed
                ? [seed.productsRes, seed.flashRes]
                : await Promise.all([productsPromise, flashPromise]);
    ...
    }, [selectedState, priceRange, sortBy, searchQuery, takeSeed]);
