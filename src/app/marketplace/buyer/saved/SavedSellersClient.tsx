 ...
 */
export default function SavedSellersClient({ initial = null }: {
    /**  #551 The saved sellers the server already fetched. */
    initial?: SavedSellerRecord[] | null;
}) {
    const [loading, setLoading] = useState(initial === null);
    const [error, setError] = useState<string | null>(null);
    const [sellers, setSellers] = useState<SavedSellerRecord[]>(initial ?? []);

    useEffect(() => {
        //   #551 Already supplied by the server. A failed read passes null and
        //   this runs, so "Could not load your saved sellers" still appears —
        //   #307's rule, a failed list is not an empty one.
