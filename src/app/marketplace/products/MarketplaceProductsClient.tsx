...
export default function MarketplaceProductsClient({ initial = null }: {
    /**
     *   #554 The FIRST, unfiltered page. A take-once seed, guarded — this
     *   screen refetches on category and on a debounced search and pages with a
     *   cursor, so anything but the initial unfiltered load asks the server.
     */
    initial?: Awaited<ReturnType<typeof getMarketplaceProductsAction>> | null;
}) {
    ...
    const fetchProducts = useCallback(async (isReset: boolean = false) => {
        try {
            ...
            const seeded = (isReset && !filterCategory && !debouncedSearch)
                ? takeSeed()
                : null;
            const result = seeded ?? await getMarketplaceProductsAction({
                limit: 12,
    ...
    }, [filterCategory, debouncedSearch, lastId, takeSeed]);
