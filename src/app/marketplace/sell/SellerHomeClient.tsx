...
export default function SellerHomeClient({ initial = null }: {
    /**
     *   #555 The two RAW results, already fetched in parallel.
     *
     *   Raw and take-once. #492 recorded what a failed read costs here — a
     *   seller with a full catalogue shown an empty shop, because the stats are
     *   computed straight off `products` — so the client keeps its own
     *   success/failure handling and `loadSellerData` stays for the refresh
     *   after a product is edited.
     */
    initial?: { productsRes: any; verificationRes: any } | null;
}) {
    ...
    const loadSellerData = async () => {
        ...
        try {
            const seed = takeSeed();
            const [productsRes, verificationRes] = seed
                ? [seed.productsRes, seed.verificationRes]
                : await Promise.all([
                            ...
                            >
                                ...
                                <div className="p-5">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button
                                            onClick={() => router.push(`/marketplace/seller/products/${product.id}/edit`)}
