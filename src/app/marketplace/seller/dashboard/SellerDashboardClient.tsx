...
export default function SellerDashboardClient({ initial = null }: {
    ...
    initial?: {
        //   Typed FROM the actions rather than as `any`. A plain `any` here
        //   silently widened the destructured results and the order comparators
        //   below lost their parameter types — tsc caught it, which is the only
        //   reason this is not a quiet regression in a sort that #439's comment
        //   right beside it says was already wrong once.
        analyticsRes: Awaited<ReturnType<typeof getSellerAnalyticsAction>>;
        ordersRes: Awaited<ReturnType<typeof getSellerOrdersAction>>;
        productsRes: Awaited<ReturnType<typeof getSellerProductsAction>>;
    ...
    useEffect(() => {
        async function loadDashboardData() {
            try {
                const seed = takeSeed();
                const [analyticsRes, ordersRes, productsRes, togglesRes] = seed
                    ? [seed.analyticsRes, seed.ordersRes, seed.productsRes, seed.togglesRes]
                    : await Promise.all([
