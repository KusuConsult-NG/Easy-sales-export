...
export default function SellerOrdersClient({ initial = null }: {
    /**
     *   #552 The FIRST, unfiltered page of orders, already fetched.
     *
     *   A take-once seed and not a state seed, because this effect re-runs on
     *   every status filter and every debounced search — and it also pages. The
     *   seed answers the first, unfiltered load and nothing else; every filter
     *   change and every "load more" fetches, exactly as before.
     */
    initial?: Awaited<ReturnType<typeof getSellerOrdersAction>> | null;
}) {
    //   Declared before fetchOrders, which reads it.
    ...
    /**
     *   #588 — a read that FAILED, as opposed to one that found nothing. The
     *   catch and the `else if (result.error)` below both wrote a log line and
     *   stopped, leaving `orders` at `[]`, so a seller with orders in flight
     *   was told "No orders found".
     */
    ...
    const fetchOrders = useCallback(async (isReset: boolean = false) => {
        try {
            ...
            //   The seed covers the first unfiltered reset only. A filtered or
            //   searched load must ask the server, or the screen would show
            //   unfiltered rows under a filter.
            const seeded = (isReset && !filterStatus && !debouncedSearch)
                ? takeSeed()
                : null;
            const result = seeded ?? await getSellerOrdersAction({
                limit: 20,
    ...
    }, [filterStatus, lastId, debouncedSearch, takeSeed]);
