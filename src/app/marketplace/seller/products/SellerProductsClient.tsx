...
export default function SellerProductsClient(
    ...
    const fetchProducts = useCallback(async (isReset: boolean = false) => {
        try {
            /**
             *   #558 THE SEED IS ONLY GOOD FOR THE FIRST PAGE, AND ONLY ONCE.
             *
             *   This effect re-runs on every filter change and on every
             *   debounced keystroke, and it reuses the same reset path. The
             *   server read "all products, no search", so a later reset asking
             *   a DIFFERENT question must not be answered with it.
             *
             *   The take-once hook is what guarantees that: the mount consumes
             *   the seed and every reset after it fetches. This was originally
             *   also guarded on `filterStatus === "all" && !debouncedSearch`,
             *   and MUTATION TESTING SHOWED THAT GUARD COULD NOT FAIL — removing
             *   it changed no test, because by the time any filter can differ
             *   the seed is already spent. An unfalsifiable check claims a
             *   protection the code is not actually getting, which is the exact
             *   thing #557 removed three commits ago; so it is gone, and what
             *   the seed depends on instead — that the filter starts unfiltered
             *   — is asserted directly in the suite.
             *
             *   `isReset` still matters: Load More must never be handed page one.
             */
            const seeded = isReset ? takeSeed() : null;
            ...
            const result = seeded ?? await getSellerProductsAction({
                limit: 20,
    ...
    }, [filterStatus, debouncedSearch, lastId, takeSeed]); // lastId dependency is tricky here
