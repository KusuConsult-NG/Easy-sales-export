...
type VerificationSeed = Awaited<ReturnType<typeof getSellerVerificationAction>>;

export default function SellerVerificationClient(
    { initial = null }: { initial?: VerificationSeed | null },
) {
    ...
    //   #559 — the server already read this. Seeded so the first paint shows
    //   the application the seller already has, and taken once so a later run
    //   reads the real thing.
    const takeSeed = useServerSeed(initial);

    const [existingVerification, setExistingVerification] = useState<any>(
        initial?.success ? initial.data?.verification ?? null : null,
    );
    ...
    useEffect(() => {
        async function checkVerification() {
            const result = takeSeed() ?? await getSellerVerificationAction();
