...
export default function VillageMarketClient({ initial = null }: {
    /**
     *   #552 The events the server already fetched.
     *
     *   This action returns the array directly rather than a {success,data}
     *   envelope, so there is no refusal to distinguish from a failure — the
     *   server page catches a throw and passes null.
     */
    initial?: VillageMarketEvent[] | null;
}) {
    const [events, setEvents] = useState<VillageMarketEvent[]>(initial ?? []);
    const [loading, setLoading] = useState(initial === null);

    useEffect(() => {
        //   #552 Already supplied by the server.
