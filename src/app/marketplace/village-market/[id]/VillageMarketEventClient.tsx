...
type EventSeed = Awaited<ReturnType<typeof getVillageMarketEventAction>>;

export default function VillageMarketEventClient({ initial = null }: { initial?: EventSeed | null }) {
    ...
    //   #559 — the server already read this event and its flash sale. Seeded so
    //   the first paint carries them, and taken once so joining the event or
    //   adding a product still refetches.
    const takeSeed = useServerSeed(initial);

    const [event, setEvent] = useState<VillageMarketEvent | null>(initial?.event ?? null);
    const [products, setProducts] = useState<FlashSaleProduct[]>(initial?.products ?? []);
    ...
    async function loadEvent() {
        //   The seed answers for the id the server rendered. A route change to
        //   another event re-runs this effect, the seed is spent, and it fetches.
        const seeded = takeSeed();
        if (!seeded) setLoading(true);
        setLoadError(null);
        try {
            const res = seeded ?? await getVillageMarketEventAction(eventId);
            setEvent(res.event);
