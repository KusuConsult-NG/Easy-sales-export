...
export default function VillageMarketSellerClient(
    { initial = null }: { initial?: VillageMarketEvent[] | null },
) {
    const { data: session, status } = useSession();
    const router = useRouter();
    const { showToast } = useToast();

    //   #559 — the server already read the active events. Taken once, so
    //   joining an event or adding a product still refetches.
    ...
    //   useCallback, not a plain function declaration: it closes over the
    //   take-once seed now, so the effect really does depend on it.
