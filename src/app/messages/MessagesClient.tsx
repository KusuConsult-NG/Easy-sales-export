...
type ConversationsSeed = Awaited<ReturnType<typeof getConversationsAction>>;

export default function MessagesClient({ initial = null }: { initial?: ConversationsSeed | null }) {
    ...
    //   #560 — the server already read the conversation list. Taken once, so
    //   the eight-second poll below still refreshes it from then on.
    ...
    useEffect(() => {
        ...
        async function fetchConversations() {
            try {
                const result = takeSeed() ?? await getConversationsAction();
                if (!isMounted) return;
                if (result.error) {
        ...
        /**
         *   THE IMMEDIATE TICK STAYS, AND IS WHAT CONSUMES THE SEED.
         *
         *   #558 had to pass `immediate: false` on /dashboard/notifications,
         *   because there the seed goes into useState and the poller's own load
         *   would have re-asked the server 0ms after hydrating. Here the seed
         *   goes INTO fetchConversations, so the first tick costs no round trip
         *   at all — and suppressing it would leave a seeded list showing a
         *   spinner for eight seconds.
         *
