...
/** The one place the action's answer is turned into what the screen shows. */
type StatusResult = { success?: boolean; data?: BankAccountStatus | null; error?: string | null };

function fromResult(result: StatusResult): { status: BankAccountStatus | null; error: string | null } {
    if (result.success && result.data) return { status: result.data, error: null };
    //   #313 — a failed check is a failure, not "not confirmed". Kept as one
    //   function so the server seed and the client refetch cannot drift.
    return { status: null, error: result.error || "Could not read your bank account" };
}
 ...
 */
export default function BankAccountClient({ initial = null }: { initial?: StatusResult | null }) {
    const { showToast } = useToast();

    //   #556 — the server already read this. Seeded into state so the first
    //   paint carries the banner, and taken once so re-verifying still refetches.
    const seed = initial ? fromResult(initial) : null;

    const [status, setStatus] = useState<BankAccountStatus | null>(seed?.status ?? null);
    ...
    const load = useCallback(async () => {
        const result = takeSeed() ?? await getBankAccountStatusAction();
