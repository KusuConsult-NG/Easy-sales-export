...
export default function LiveTrainingClient(
    { initial = null }: { initial?: TrainingSession[] | null },
) {
    const { data: session, status: sessionStatus } = useSession();

    //   #567 — the server read the schedule through the same gated function the
    //   route uses. Handed INTO the loader, because this screen DERIVES from
    //   it: it picks out whichever session is live right now, and that clock
    //   arithmetic belongs on the side that has the viewer's clock.
    ...
    useEffect(() => {
        ...
        const fetchSessions = async () => {
            try {
                ...
                const res = seeded ? null : await fetch("/api/wave/training-sessions");
                if (seeded || res!.ok) {
                    const data = seeded ? { data: { sessions: seeded } } : await res!.json();
        ...
        /**
         *   THE IMMEDIATE TICK STAYS — #560's rule.
         *
         *   The seed goes INTO fetchSessions, so the first tick is what
         *   consumes it: no round trip, and the "which session is live now"
         *   arithmetic runs against the viewer's own clock straight away.
         *   Suppressing it would leave a seeded screen on a spinner for a full
         *   minute.
         *
