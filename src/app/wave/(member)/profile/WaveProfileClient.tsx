 ...
 *
 * `stats` is null for a member the screen is about to turn away — there is no
 * point reading statistics for somebody being redirected to /wave — and the
 * client never reaches its read in that case either.
 */
export type WaveProfileSeed = {
    membership: Awaited<ReturnType<typeof checkWaveMembershipAction>>;
    stats: Awaited<ReturnType<typeof getWaveMemberStatsAction>> | null;
};

export default function WaveProfileClient({ initial = null }: { initial?: WaveProfileSeed | null }) {
    ...
    //   #556 — the server walked this chain already. Taken once: every later
    //   run of loadProfile must fetch, or the screen would show page-load data
    ...
    async function loadProfile() {
        ...
        try {
            ...
            // Load stats
            const statsResult = seed?.stats ?? await getWaveMemberStatsAction();
