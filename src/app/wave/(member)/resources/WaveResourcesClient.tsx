...
export type WaveResourcesSeed = {
    eligibility: Awaited<ReturnType<typeof checkWaveEligibilityAction>>;
    /** Null only when the eligibility answer sends this visitor away. */
    resources: Awaited<ReturnType<typeof getResourcesAction>> | null;
};
...
export default function WaveResourcesClient({ initial = null }: { initial?: WaveResourcesSeed | null }) {
    ...
    //   #556 — the server already made both of these reads. Two take-once
    //   seeds rather than one, because two separate effects consume them.
    const takeEligibility = useServerSeed(initial?.eligibility ?? null);
    const takeResources = useServerSeed(initial?.resources ?? null);
    ...
    useEffect(() => {
        ...
        async function checkEligibility() {
            setChecking(true);
            const eligibility = takeEligibility() ?? await checkWaveEligibilityAction(userId as string);

            /**
             *   #557 THIS DOOR ONLY EVER OPENED THE WRONG WAY.
             *
             *   It was `if (!eligibility.success) router.push("/wave/application")`
             *   and it never read `eligibility.data.eligible` at all. So the
             *   only thing it did was throw a member out of the member area
             *   when the check FAILED — the #323 shape again, "could not tell"
             *   read as "no" — while an answer of "not eligible" walked
             *   straight through, because that answer comes back success:true.
             *
             *   Nothing was protected by it. This route sits under
             *   wave/(member)/layout.tsx, which resolves the session and calls
             *   checkModuleAccess before any of this renders, and the library
             *   itself is behind callerMayAccessResources — an ACTIVE
             *   wave_members row or an admin — inside getResourcesAction. Two
             *   correct gates; this was a third that enforced nothing and
             *   ejected people.
             *
             *   Now all three states are told apart: a real "not eligible" goes
             *   to the application page, a failed check says so and stays put,
             *   and a member gets their library.
             */
            if (!eligibility.success) {
                showToast(
                    eligibility.error || "Could not check your WAVE eligibility",
                    "error",
                );
                setChecking(false);
                return;
            ...
            if (eligibility.data && eligibility.data.eligible === false) {
    ...
    useEffect(() => {
        async function loadResources() {
             ...
             */
            try {
            const data = takeResources() ?? await getResourcesAction();
