 ...
 *
 * `events` and `registrations` are null for a visitor the screen is about to
 * send back to /wave — the client never reaches those reads in that case
 * either.
 */
export type WaveTrainingSeed = {
    membership: Awaited<ReturnType<typeof checkWaveMembershipAction>>;
    events: Awaited<ReturnType<typeof getWaveTrainingEventsAction>> | null;
    registrations: Awaited<ReturnType<typeof getUserTrainingRegistrationsAction>> | null;
};

export default function WaveTrainingClient({ initial = null }: { initial?: WaveTrainingSeed | null }) {
    ...
    async function loadTrainingData() {
        ...
        try {
            // Check membership
            const membership = seed?.membership ?? await checkWaveMembershipAction();
            ...
            //   The events and this member's registrations have nothing to do
            //   with each other, and were awaited one after the other — so the
            //   member waited the SUM of two round trips rather than the longer
            //   of them. Same fix as #543 gave the WAVE dashboard.
            const [eventsResult, regsResult] = seed?.events && seed.registrations
                ? [seed.events, seed.registrations]
                : await Promise.all([
                    getWaveTrainingEventsAction(undefined, 100, true),
