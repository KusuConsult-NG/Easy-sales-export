 ...
 *
 * `application` and `access` follow the SAME branch the client takes, so each
 * is null unless the status the server saw actually calls for it. Nothing is
 * read speculatively.
 */
export type WaveApplicationSeed = {
    status: Awaited<ReturnType<typeof checkWaveStatusAction>>;
    application: Awaited<ReturnType<typeof getWaveApplicationAction>> | null;
    access: Awaited<ReturnType<typeof checkWaveAccessAction>> | null;
};

export default function WaveApplicationClient(
    { initial = null }: { initial?: WaveApplicationSeed | null },
) {
    ...
    const checkApplicationStatus = async (isEditParam: boolean) => {
        ...
        //   branches. Taken once, so a session change does the real reads.
        const seed = takeSeed();
        try {
            const statusResult = seed?.status ?? await checkWaveStatusAction();
            ...
            if (waveStatus === "pending" || waveStatus === "under_review") {
                ...
                const hasAccessResult = seed?.access ?? await checkWaveAccessAction();
