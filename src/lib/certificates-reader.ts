 ...
 *
 *   #562 /dashboard/certificates ASKED ITS OWN SERVER FOR THESE OVER HTTP.
 *
 *   It is a "use client" page whose first act on mount was two `fetch` calls
 *   back to this same application — /api/certificates and
 *   /api/academy/certificates — after the page had already been rendered,
 *   downloaded and hydrated. The bodies of both handlers move here so the
 *   server page can read them directly, and both routes now call these too:
 *   one definition, two callers each, rather than a copy per caller.
 *
 *   These are the SAME reads the routes performed, with the same session-scoped
 *   filtering. The caller supplies the user id; neither function widens what a
 *   member may see, and neither is reachable without one.
 */
...
/**
 * The certificates a member uploaded themselves.
 *
 *   #303 A certificate the member removed leaves this list. The row and the
 *   stored file both survive — see the DELETE route — so the filter here is
...
/**
 * The certificates a member EARNED — Academy course completions and WAVE.
 *
 * Cursor is the issuedAt of the last item returned; `limit` is clamped to
 * 1..50 by the caller that parses it.
 */
export async function readAcademyCertificates(
    userId: string,
    options: { limit?: number; cursor?: string | null } = {},
): Promise<{ certificates: AcademyCertificate[]; cursor: string | null; hasMore: boolean }> {
    const limit = Math.min(Math.max(options.limit ?? 20, 1), 50);
    const cursorParam = options.cursor ?? null;

    type Certificate = AcademyCertificate;
    ...
    return { certificates: page, cursor: nextCursor, hasMore };
}
