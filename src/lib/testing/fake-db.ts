...
export interface FakeDbHandle {
    ...
    generatedIds: string[];
    /**
     * Every read this store served, in order — one entry per ROUND TRIP.
     *
     *   "THE ENTIRE APP IS STILL VERY SLOW", said after the admin side was
     *   measured and repaired and the user side never was.
     *
     *   Latency on this platform is round trips, not CPU: each `.get()` is one
     *   HTTP call to PostgREST, and the adapter has no request-scoped identity
     *   map, so the same row asked for twice is fetched twice. That cost is
     *   invisible in the source — the second read is in a different module
     *   from the first, which is exactly why it survives review — and it is
     *   trivially visible here.
     *
     *   `id` is null for a query, which is one round trip too (and may be more
     *   than one when it pages; see the adapter's 1,000-row batches).
     *
     *   Reset between phases with `reads.length = 0`; `clear()` leaves it
     *   alone, because emptying the STORE and forgetting what was ASKED are
     *   different things and a test usually wants only the first.
     */
    reads: ReadRecord[];
}

/** One round trip the fake served: a keyed document read, or a query. */
export interface ReadRecord {
    collection: string;
    /** The document id, or null when the read was a query. */
    id: string | null;
}
...
export function installFakeDb(seed: Record<string, Record<string, Doc>> = {}): FakeDbHandle {
    ...
    g.mockFirestoreGet.mockImplementation(() => {
        ...
        //   Recorded BEFORE the branch, so a query and a keyed read cost the
        //   same one entry — they cost the same one round trip.
        reads.push({ collection: d.collection ?? '', id: d.kind === 'doc' ? (d.id ?? null) : null });
