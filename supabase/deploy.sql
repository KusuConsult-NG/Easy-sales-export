-- ============================================================================
-- CONSOLIDATED DEPLOY — 50 migrations
...
-- ============================================================================
-- 046 — 046_consolidate_wallet_to_live_profile.sql
-- the way OUT of a stranded wallet balance — one filed under a profile nobody signs in as, which no member, checkout or withdrawal can reach. Money moves at the LIVE id and nowhere else: 005's credit and debit both key on p_user_id, and a session id is live by construction (#490). The repair has to be ONE function because a credit and a debit as two calls are each idempotent but NOT atomic as a pair, and a credit that lands without its debit mints money. It carries its own authorisation — the _migratedTo / supabaseAuthId pointer is re-read inside the same transaction and any pair the platform does not already call one person is refused — so it can never be a general transfer primitive. Writes both copies of the balance, per 011. MUST come after 011, whose pattern it follows. Applying it moves nothing; it is only callable deliberately.
-- ============================================================================
...
-- ============================================================================
-- 047 — 047_user_search_and_purge_indexes.sql
-- the two `users` scans that time out. 044 and 045 fixed the rollup; these are the other two 57014s. searchUserIdsByQuery fires up to SEVENTEEN queries per admin search, TWELVE of them name prefix ranges across fullName/firstName/lastName — none of which is a native column and none of which had an index, while 036 had indexed the phone half of the same helper. The GDPR purge filters deletedAt <= threshold AND gdprPurgedAt IS NULL, both inside raw_data, neither indexed. Measured at 40k rows, 54 MB: name prefix 8,635 buffers -> 32 (270x), purge 6,667 -> 11 (600x). The purge index is PARTIAL on being soft-deleted, because nearly every row has a null gdprPurgedAt and indexing that alone would cover the table — 16 kB against 1,248 kB for each name index. Order does not matter: four indexes, no table rewritten, no row touched, every statement IF NOT EXISTS.
-- ============================================================================
...
-- ============================================================================
-- 048 — 048_the_indexes_022_could_not_deploy.sql
-- the six indexes 022 declared and no deploy could ever apply. 022 writes all nine with CREATE INDEX CONCURRENTLY, which cannot run inside a transaction block, which is why 022 is in EXCLUDED and has to be applied BY HAND. 041 and 043 each rescued ONE into the plain form under 022's own name; six were never rescued, among them the (collection_name, status) and (collection_name, userId) pair 041's own header calls "the two that cover most of the application", and the reference every payment looks a marketplace order up by. Measured at 200k document_collections rows: a user's own rows in a collection 23,963 buffers -> 4, an order by reference 2,223 -> 4. NOT rescued: 022's two cooperative_members indexes, because FIELD_TO_COLUMN routes both names to native columns so the JSONB path is never emitted and neither could be reached. Order does not matter: six indexes, no table rewritten, no row touched, every statement IF NOT EXISTS on 022's own names, so it is a no-op wherever 022 was applied.
-- ============================================================================
...
-- ============================================================================
-- 049 — 049_count_module_registrations.sql
-- count_module_registrations — the admin dashboard's module usage breakdown in ONE table scan. analytics.service issued EIGHT count queries against users, each an OR of a JSONB path and a roles containment; nothing indexes the JSONB path and an OR cannot be served by an index on one arm, so all eight were sequential scans that ALSO detoasted the whole of raw_data per row to read one short string — 044 and 045's finding, which this path never learned from. Measured at 42,845 rows with raw_data genuinely in TOAST: one of the eight 129,642 buffers / 162 ms, all eight ~1,037,000 / ~1.3 s, this function 1,262 / 42 ms. Reads 045's generated service_regs column, so no row is detoasted. MUST come after 045, which adds that column. The status lists are stated here AND in lib/module-registration-status.ts, held together by __tests__/pg/the-module-counts-agree-with-the-javascript, which rebuilds all eight predicates from the TypeScript and fails on a single disagreement. analytics.service falls back to the eight queries when the function is absent, so a deploy landing before this migration is slow rather than broken.
-- ============================================================================
...
-- ============================================================================
-- 051 — 051_users_raw_created_at_index.sql
-- idx_users_raw_created_at — the admin dashboard's user-growth chart, six month buckets at once, every one of which timed out in production: '[DashboardStats] user growth count failed ... canceling statement due to statement timeout'. 027 already put (created_at DESC) on this table, and it CANNOT serve this query: `createdAt` is in neither FIELD_TO_COLUMN['users'] nor NATIVE_COLUMNS['users'], so supabase-db emits raw_data->>'createdAt' and a btree on the column cannot answer a predicate on the JSON key. Mapping the field to the column instead would be the WRONG fix: _mapRow reads raw_data?.createdAt ?? created_at, so the key is authoritative and the chart would silently bucket by a different value from the one each row displays. Measured on 40,000 seeded users in a 107 MB table (production is 42,845 / 106 MB): seq scan 10,000 buffers / 20.9 ms, bitmap index 1,115 buffers / 2.6 ms, and the planner's estimate for the bucket went from rows=200 to rows=4,065 against an actual 3,996 — an expression index carries statistics a bare JSONB expression does not. Changes no row and is not required for correctness, so a deploy landing before it is slow rather than broken.
-- ============================================================================
...
-- ============================================================================
-- 052 — 052_missing_schema_objects.sql
-- missing_schema_objects — the function that answers 'is the schema this code was written against the schema it is running against?', which nothing could answer. The day it was written: a `count users` fix merged at midday, the admin dashboard reading 'Total Users — could not be read' all afternoon, and the reason being that the fix had two halves. The app half shipped; the other half was 051, one CREATE INDEX, and nothing applies these migrations to production — not the Dockerfile, not deploy-production.yml, not a start script. CI runs them against a throwaway cluster, so every suite was green against a database that had all of them while the real one was missing two. Takes the names lib/migration-manifest expects and returns the ones pg_indexes and pg_proc do not have; api/cron/migration-audit answers 503 while any are missing, naming the files to paste. Reads catalogs only, writes nothing, and MUST come last of the additive steps so that its own first run reports on a database the rest of this file has already brought up to date. Its absence is the most informative answer it has: the audit reports cannot-tell and names this file.
-- ============================================================================
...
-- 1. Every function should be listed. Expect 38 rows.
--
...
SELECT proname
 ...
 WHERE proname IN (
    ...
    'claim_status_transition_in',
    'claim_versioned_update',
    'consolidate_wallet_to_live_profile',
